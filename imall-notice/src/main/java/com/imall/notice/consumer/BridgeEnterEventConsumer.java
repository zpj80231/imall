...
@RabbitListener(bindings = @QueueBinding(
        value = @Queue(value = MqConstant.Queue.BridgeEnterEvent, durable = "true", autoDelete = "false"),
        exchange = @Exchange(value = MqConstant.Exchange.EVENT, type = ExchangeTypes.TOPIC),
        key = MqConstant.RoutingKey.BridgeEnterEvent
))
@Slf4j
public class BridgeEnterEventConsumer {
    ...
    public void topicreviceMessage(String msg, Channel channel, Message message) {
        try {
            ...
            String webMsg = callerIdNum + "接听电话, [" + eventId + "]";
